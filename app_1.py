...
def main():
    ...
    st.markdown("<h1 style='text-align: center; color: #8B0000;'>Áreas Naturales Protegidas (ANP) de Administración Nacional Definitiva </h1>", unsafe_allow_html=True)
    ...
    fig_anp_cate.update_traces(marker_color='#ADFF2F', marker_line_color='rgb(8,48,107)', marker_line_width=1.5)
    ...
    fig_distrito.update_traces(marker_color='#ff7f0e', marker_line_color='rgb(8,48,107)', marker_line_width=1.5)
    ...
    st.markdown("<h2 style='color: #20B2AA;'>Mapa de Ubigeos</h2>", unsafe_allow_html=True)
    st.markdown("En esta sección de esta página estamos presentando la ubicación de distintas áreas naturales protegidas a nivel nacional representándolos por íconos de mapeo.")
    folium_static(mapa_ubigeos)
    st.markdown("<h2 style='color: #20B2AA;'>Gráficas</h2>", unsafe_allow_html=True)
